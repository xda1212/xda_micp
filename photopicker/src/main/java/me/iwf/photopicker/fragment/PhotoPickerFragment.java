 ...
 */
public class PhotoPickerFragment extends Fragment implements OnClickListener{
  ...
  @Override public void onCreate(Bundle savedInstanceState) {
    ...
    maxCount = getArguments().getInt(EXTRA_COUNT);
...
//    final Button btSwitchDirectory = (Button) rootView.findViewById(R.id.button);
    ...
    final TextView tvShowAll = (TextView) rootView.findViewById(R.id.tv_show_all);
    RelativeLayout headLayout = (RelativeLayout) rootView.findViewById(R.id.head_layout);
    tvShowAll.setOnClickListener(this);

    listPopupWindow = new ListPopupWindow(getActivity());
    listPopupWindow.setBackgroundDrawable(getResources().getDrawable(R.drawable.__picker_item_bg));
    listPopupWindow.setWidth(ListPopupWindow.MATCH_PARENT);
    listPopupWindow.setAnchorView(headLayout);
    ...
    listPopupWindow.setOnItemClickListener(new AdapterView.OnItemClickListener() {
      @Override public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        tvShowAll.setText(directory.getName());
  ...
  @Override public void onActivityResult(int requestCode, int resultCode, Intent data) {
    if (requestCode == ImageCaptureManager.REQUEST_TAKE_PHOTO && resultCode == RESULT_OK) {
      ...
      if (directories.size() > 0) {
        ...
        photoGridAdapter.notifyDataSetChanged();
        if (photoGridAdapter.getSelectedPhotos().size() < maxCount) {
          photoGridAdapter.getSelectedPhotos().add(path);
          photoGridAdapter.notifyDataSetChanged();
          PhotoPickerActivity activity = (PhotoPickerActivity) getActivity();
          activity.getAdapter().getDataList().add(path);
          activity.getAdapter().notifyDataSetChanged();
          activity.setTextCount(photoGridAdapter.getSelectedPhotos().size());
        }else {

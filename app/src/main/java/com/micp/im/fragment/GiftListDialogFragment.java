...
import com.micp.im.adapter.GridViewAdapter;
import com.micp.im.adapter.ViewPageGridViewAdapter;
...
//        }
    ...
    private void doSendGift(final String isOutTime) {
        if (mSelectedGiftItem != null) {
            ...
            if (getActivity() instanceof VideoPlayerActivity && ((VideoPlayerActivity) getActivity()).mConnectionState) {
                ...
                doSendGiftWithoutLive();
            }
        }
    }

    private void doSendGiftWithoutLive() {
        PhoneLiveApi.sendGiftWithoutLive(mUser, mSelectedGiftItem, ((HomePageActivity) getActivity()).uid, new StringCallback() {
            @Override
            ...
            public void onResponse(String response, int id) {
                ...
                if (s != null) {
                    try {
                        ...
                        AppContext.showToastAppMsg(getActivity(), "赠送成功");
...
//        if (getActivity() instanceof VideoPlayerActivity && ((VideoPlayerActivity) getActivity()).mConnectionState) {//没有连接ok

...
import com.micp.im.ui.customviews.FixGridLayout;
...
//    private SHARE_MEDIA mShare_meidia = SHARE_MEDIA.MORE;
    ...
    private void uploadVideoToQiNiu(final String title) {
        ...
        UploadUtil.getInstance().uploadVideo(datas, new UploadUtil.Callback() {
            @Override
            public void callbackSuccess(UploadBean bean) {
                TLog.error("run: ----上传七牛云成功-->");
                requestUpload(title, bean);
            }

            @Override
            public void callbackFail(UploadBean bean) {
    ...
    private void addShareView() {
        ...
        for (int i = 0; i < AppConfig.SHARE_TYPE.length(); i++) {
            ...
            try {
                im.setImageResource(getResources().getIdentifier(AppConfig.SHARE_TYPE.getString(i) + "_share", "drawable", "com.micp.im"));
            } catch (JSONException e) {
                e.printStackTrace();
            }
